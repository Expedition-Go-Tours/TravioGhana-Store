...
export function useTravelerSelection(tour: TravelerSelectionTour, options?: TravelerSelectionOptions) {
  ...
  // When the caller seeds exact per-category counts (e.g. restoring a booking's
  // mix on the change/modify flows) never overwrite them when pricing arrives
  // asynchronously — only the unseeded default widget reseeds from the minimum.
  const hasProvidedInitialCounts =
    !!(options?.initialCounts && Object.keys(options.initialCounts).length > 0)
  if (!isPerGroup && !hasProvidedInitialCounts && travelerGroups.length > 0 && prevCategories !== travelerGroups) {
